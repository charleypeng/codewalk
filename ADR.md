...
Issue #90 extends the same surface with a focused text/code editor and explicit save: the file viewer (ADR-008) is upgraded into a dirty-tracked editor with an explicit `Save` action that writes back to the host filesystem through a fifth shell-scripted operation. The save path reuses every containment rule, ephemeral-session lifecycle, and capability probe already defined above — it is not a parallel mutation surface, just one more one-shot script under the same `CW_FILE_OP_JSON:` contract. ADR-008's viewer stays the read-only fallback when the capability is missing; issue #90 only adds a `Save` affordance and a draft state machine, never a new mutation channel.
...
2. **Hidden ephemeral OpenCode shell session per call.** Each mutation opens a fresh ephemeral session via `POST /session` (title: `ChatTitleGenerator.ephemeralSessionTitle`, which the realtime layer already filters out — ADR-009) with the active project directory sent through official `WorkspaceRoutingQuery`, executes a small POSIX shell script via `POST /session/:id/shell` with the same `directory` and `data: { 'agent': 'build', 'command': <script> }`, and tears the session down with `DELETE /session/:id` carrying the same routing directory in a `finally` block. Official OpenCode session routing stores `session.directory` at creation and prefers it for session-scoped calls, so creation, shell execution, and cleanup remain pinned to the same project root. The session id is also added to `ChatTitleGenerator.ephemeralSessionIds` so the chat provider can ignore the session in the realtime stream and the title generator's 5-second delayed prune keeps the id out of leak paths. There is no PTY, no terminal endpoint, and no streaming output — this is one-shot command execution, not a shell session.
...
- **Editor + explicit save reuses the ADR-043 transport, not a new contract.** The `writeFile` operation opens the same ephemeral shell session and runs a fifth one-shot POSIX script over the existing `CW_FILE_OP_JSON:` sentinel; the file body rides as a single `CW_CONTENT_B64` shell variable and is decoded inside the same helpers (`base64` / `base64 --decode` with a `python3` fallback). No new endpoint, no new request schema, no new event semantic — and the read-only fallback (`shellFileOpsSupported == false`) is the same gate the four pre-existing mutations already use.
...
- **Base64 + `cw_decode_content` avoids heredocs and shell-quoting pitfalls.** The editor only carries UTF-8 text content (the `CodeEditor` buffer is text), so `base64Encode(utf8.encode(content))` produces a deterministic, well-formed payload that the host can decode with `base64` / `base64 --decode` (and a `python3` fallback for stripped images where neither binary is guaranteed). The single-shell-variable + single-decoder-call pattern is the same single-line posture the ADR-029 truncation post-mortem recommended — no heredocs, no unquoted interpolation.
- **Large-file + dirty-tab UI gates keep the editor honest.** Opening a file `> 1 MiB` in read-only mode and refusing a silent tab reload while a draft is dirty (handled by `_reloadFileTab`, the same path used for diff-aware reloads) are client-side guards that mirror the existing capability-gated affordances. The static `'Unsaved changes; reload skipped.'` string set on `saveErrorMessage` — rather than an ARB-localizable key — keeps the editor self-contained and makes "the save did not happen" impossible to miss without inventing a new translation surface; together with the inline + snackbar save-error reporting the editor never silently loses state.
- **Draft-length save gating beats original-content gating.** Gating `_canSaveFileDraft` against the **current draft UTF-8 byte length** rather than the original on-disk content catches the case where the user pastes / types past the 1 MiB ceiling *after* the file was loaded; the static `'Draft is too large to save from the editor.'` string is a non-localizable editor-local message that always reflects the real blocker, and the shell call is suppressed so the host never sees an over-cap write.
...
- ✅ Save gating uses the current draft UTF-8 byte length, so an oversize paste/type past the 1 MiB ceiling is blocked at the editor boundary (with the static `'Draft is too large to save from the editor.'` message) and never dispatches a shell call.
...
- ⚠ `writeFile` is a full round-trip per save (`POST /session` → `POST /session/:id/shell` → `DELETE /session/:id`) with the editor's UTF-8 text content base64-encoded into the shell command, so very large files add base64 inflation on top of the network latency. The 1 MiB read-only cap keeps the worst-case save command inside a sane shell-arg size; bulk write paths or streaming edits remain out of scope behind a future ADR.
...
**Why this is not a free pass for additional divergences**: The exception is scoped to the four mutation operations, the focused-editor `writeFile` save (issue #90), and the capability probe. The save path is in-scope because it is one more one-shot shell script over the same `CW_FILE_OP_JSON:` sentinel and the same `shellFileOpsSupported` gate — no new endpoint, no new request schema, no new event semantic. Any new mutation, any cross-directory move, any bulk operation, any autosave / diff / format-on-save on the editor, or any read-side change must be evaluated against ADR-023 on its own merits. Future shell-script growth must be reviewed the same way ADR-029 was reviewed (and the ADR-029 truncation post-mortem still applies: keep scripts short, avoid heredocs, prefer structured one-liners).
...
- **Low routing risk.** Creation, shell execution, and cleanup must continue to carry the same official `WorkspaceRoutingQuery` directory. Dropping one leg could route a session-scoped call outside the active project. Mitigation: OpenCode stores `session.directory`, and the service sends the directory on every ephemeral file-operation call.
...
- **Low UX risk.** A successful mutation that the user did not see (e.g. a delete that returns `ok` because the script ran but the file was already gone) is unlikely because the script checks existence / writability before acting. Confirmed deletes reconcile absolute/relative/root-relative aliases before and after parent refresh, and pending-delete guards block matching edits/mutations until the host result. A failed delete unlocks and preserves state.
...
- `lib/presentation/services/workspace_file_operations_service.dart` — `WorkspaceFileOperationsService` interface (incl. `writeFile`), `WorkspaceFileOperationsServiceImpl` (Dio + ephemeral shell + sentinel protocol + `_buildWriteFileCommand` + `cw_decode_content` base64/python3 fallback + `tmpdir=$(mktemp -d "$parent/.cw-write.XXXXXX")` / `tmp="$tmpdir/content"` + `cw_copy_mode "$target" "$tmp"` mode preservation + `mv --` + `rm` + `rmdir` cleanup), `WorkspaceFileOperationsCapabilities`, `WorkspaceFileOperationResult`, `WorkspaceFileOperationCode` enum, `_preparePathOperation` for absolute / `..` / NUL refusal
...
- `lib/presentation/pages/chat_page/chat_page_file_viewer.dart` — focused `CodeEditor`-backed editor (`_buildFocusedFileEditor`, `_maxEditableFileLength = 1024 * 1024`, `_canSaveFileDraft`, `_editorReadOnlyReason`, dirty / saving / `savedContent` / `saveErrorMessage` rendering, inline error decoration, explicit `Save` action)
...
- `test/unit/presentation/workspace_file_operations_service_test.dart` — sentinel extraction, malformed payload mapping + cache clear, shell quoting, capability probe + cache, invalid leaf names before any shell call, root-directory refusal, probe fails closed on canonical root, mutation script rejects canonical root for non-literal roots, create-file round-trip with shell-quoted names, malformed probe disables shell file operations, and the issue #90 `writeFile` suite: base64 content transport + target-path join, `..` + absolute outside-root refusal before any shell call, project-root refusal before any shell call, and malformed `writeFile` result invalidating the capability cache.
