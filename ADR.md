...
- **Temp file under parent + atomic `mv` keeps the host authoritative.** Writing to `$parent/.cw-write-$$.tmp` and `mv --` over the target is a host-side atomic rename that reuses the project-root containment (`cw_prepare_parent` + `case "$parent" in "$root"|"$root"/*)`) the four pre-existing operations already enforce; the editor's draft state (`isDirty`, `isSaving`, `savedContent`, `saveErrorMessage`) is a UI-only cache that never touches the project filesystem, so the host stays the only writer.
...
> **Implementation behavior (not covered by the regression tests above, but still true per Decisions 1–11 above):** `_reloadFileTab` blocks silent reloads of tabs whose draft `isDirty` is true and stamps the draft with the static `saveErrorMessage = 'Unsaved changes; reload skipped.'` (not an ARB key); large files (> 1 MiB) force read-only via `_editorReadOnlyReason`; the editor enforces the `_maxEditableFileLength = 1024 * 1024` ceiling; `cw_decode_content` falls back from `base64` to `python3` when neither `base64` nor `base64 --decode` is on the host; the `mv --` over the temp file is wrapped by `rm -f -- "$tmp"` so a failed atomic rename does not leave the `$parent/.cw-write-$$.tmp` behind; targets that are missing/directories return `missing`/`notDirectory` from the shell-side checks; the focused editor's tab title carries the literal `*` dirty marker via the `file_viewer_tab_dirty_<path>` `ValueKey`; the file-tree context menu exposes `copyPath` and `refresh` even when the shell probe is unavailable.

### Key Files

- `lib/presentation/services/workspace_file_operations_service.dart` — `WorkspaceFileOperationsService` interface (incl. `writeFile`), `WorkspaceFileOperationsServiceImpl` (Dio + ephemeral shell + sentinel protocol + `_buildWriteFileCommand` + `cw_decode_content` base64/python3 fallback + `$parent/.cw-write-$$.tmp` + `mv --`), `WorkspaceFileOperationsCapabilities`, `WorkspaceFileOperationResult`, `WorkspaceFileOperationCode` enum, `_preparePathOperation` for absolute / `..` / NUL refusal
