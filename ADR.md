...
1. **Optional Cloudflare Managed OAuth flow**: Add an opt-in Cloudflare Managed OAuth authentication capability for desktop and Android platforms. The flow uses a system browser + in-app local HTTP redirect server on both desktop and Android (Cloudflare Managed OAuth DCR only allows loopback redirect URIs, so Android runs the same in-app loopback server). When enabled per server profile, CodeWalk performs the Cloudflare Managed OAuth authorization code flow with PKCE S256. The resulting access token is sent as `Authorization: Bearer <access_token>` on requests whose origin matches the OAuth-enabled profile only. When a `registration_endpoint` is available, Dynamic Client Registration (DCR) is performed to obtain client credentials automatically.
...
- ✅ Platform gating via `AppProvider.supportsCloudflareAccessOAuth` — desktop and Android use an in-app local HTTP redirect server, iOS is stub/no-op.
...
- **iOS no-op**: On iOS, `OAuthService` (stub) must be a no-op and `AppProvider.supportsCloudflareAccessOAuth` must return false for iOS; UI must not expose OAuth configuration. Android must pass `supportsCloudflareAccessOAuth` as true and the IO implementation must handle the loopback redirect flow on Android.
...
- `lib/core/auth/oauth_service_io.dart` — IO platforms (desktop + Android) implementation: desktop uses system browser + local HTTP redirect server; Android uses the same system browser + in-app local HTTP redirect server (Cloudflare DCR rejects non-loopback redirect URIs); Cloudflare Managed OAuth authorization code + PKCE S256, DCR when `registration_endpoint` available, `/oauth/callback` handling with state/path/duplicate rejection, token exchange
