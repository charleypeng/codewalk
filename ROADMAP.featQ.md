...
- [~] Group 3 — Chat UX, Composer, and Tool Bubble Polish
  ...
  - [x] Investigate and fix conversation-open click behavior: sometimes opening a conversation requires two clicks (first opens the conversation, second closes the sidebar), and a fast double-click on the same item can freeze the screen, especially with large histories. WS1: drawer closes immediately on session tap; _handleSessionSwitch guard prevents concurrent switches; session-switch overlay shows while content hydrates; loadSessionInsights runs fire-and-forget. - Related commits: 204114e 09c1641 eccec6b dfa9754
  - [x] Chat UX stability plan (WS1–WS5): WS1 drawer-immediate + session-switch guard + overlay; WS2 per-session collapse-state cache with LRU eviction (20 sessions); WS3 reconcile one-shot guard using _messageStreamGeneration turn key to ensure final response always renders; WS4 role-based bubble entrance animations (user 130ms scale+slide+fade, assistant 180ms slide+fade); WS5 regression test updated to match new collapse-persistence contract (543 tests pass). - Related commits: 09c1641 eccec6b
  - [x] Investigate and fix intermittent blank history on conversation open: chat can appear empty until pull/scroll, likely due to auto-scroll to the last message overshooting (overscroll) and leaving the viewport outside the content. Replaced with cache-first SWR using per-session LRU snapshot cache for instant restore. - Commit hash: 26c8448
  - [x] Load older history on top reach with anchor preservation: implemented top-scroll trigger to fetch previous message batches. - Commit hash: 8b364fd
  ...
  - [ ] Plan the merge between the project selector and the conversations sidebar, grouping conversations by open projects to speed up navigation. — `0cc892c`
