...
| 039 | Real Windows STT fix via WASAPI capture and actionable settings links — supersedes the ADR-038 Windows mitigation with a working `codewalk/windows_microphone` channel + `SpeechAudioCapture` routing; on-device engines re-enabled; typed `WindowsMicrophoneAccessStatus` preflight; `WindowsSettingsLinks` opens `ms-settings:privacy-microphone`, `ms-settings:privacy-speech`, `ms-settings:speech`; Native STT shows a specific reason + actionable snackbar | 2083–2208 |
