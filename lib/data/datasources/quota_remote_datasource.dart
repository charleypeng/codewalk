...
class QuotaRemoteDataSourceImpl implements QuotaRemoteDataSource {
  ...
  String _buildQuotaShellCommand() {
    ...
    const jsScript =
        ...
        "const DATA=p.join(process.env.XDG_DATA_HOME||p.join(os.homedir(),'.local','share'),'opencode');"
        "const AG=[p.join(CFG,'antigravity-accounts.json'),p.join(DATA,'antigravity-accounts.json')];"
        "const GEP=['https://daily-cloudcode-pa.sandbox.googleapis.com','https://autopush-cloudcode-pa.sandbox.googleapis.com','https://cloudcode-pa.googleapis.com'];"
        "const GHD={'User-Agent':'antigravity/1.11.5 windows/amd64','X-Goog-Api-Client':'google-cloud-sdk vscode_cloudshelleditor/0.1','Client-Metadata':'{\"ideType\":\"IDE_UNSPECIFIED\",\"platform\":\"PLATFORM_UNSPECIFIED\",\"pluginType\":\"GEMINI\"}'};"
        ...
        "function rJson(f){try{if(!fs.existsSync(f))return null;const r=fs.readFileSync(f,'utf8').trim();if(!r)return null;const j=JSON.parse(r);return j&&typeof j==='object'?j:null;}catch{return null;}}"
        ...
        "function asO(v){return v&&typeof v==='object'&&!Array.isArray(v)?v:null;}"
        "function asS(v){if(typeof v!=='string')return null;const t=v.trim();return t||null;}"
        "function pickS(){for(const v of arguments){const s=asS(v);if(s)return s;}return null;}"
        ...
        "function pgR(v){const r=asS(v);if(!r)return{refreshToken:null,projectId:null,managedProjectId:null};const parts=r.split('|');return{refreshToken:asS(parts[0]),projectId:asS(parts[1]),managedProjectId:asS(parts[2])};}"
        "function gCred(obj,prefix){const o=asO(obj)||{};const c=asO(o.client);return{clientId:pickS(o.clientId,o.client_id,o.googleClientId,o.google_client_id,c&&c.id,c&&c.clientId,prefix?process.env[prefix+'_GOOGLE_CLIENT_ID']:null,process.env.GOOGLE_CLIENT_ID),clientSecret:pickS(o.clientSecret,o.client_secret,o.googleClientSecret,o.google_client_secret,c&&c.secret,c&&c.clientSecret,prefix?process.env[prefix+'_GOOGLE_CLIENT_SECRET']:null,process.env.GOOGLE_CLIENT_SECRET)};}"
        ...
        "function gWin(s,rA){if(s==='antigravity'){const rem=typeof rA==='number'?Math.max(0,Math.round((rA-Date.now())/1000)):null;if(rem!==null&&rem>36000)return{label:'daily',seconds:86400};return{label:'5h',seconds:18000};}return{label:'daily',seconds:86400};}"
        ...
        "function rGem(a){const raw=nE(getE(a,['google','google.oauth']));const eo=asO(raw);if(!eo)return null;const oo=asO(eo.oauth)||eo;const rp=pgR(oo.refresh);const gc=gCred(oo,'GEMINI');const ec=gCred(eo,'GEMINI');const at=pickS(oo.access,oo.token,eo.access,eo.token);if(!at&&!rp.refreshToken)return null;return{sourceId:'gemini',sourceLabel:'Gemini',accessToken:at,refreshToken:rp.refreshToken,projectId:pickS(oo.projectId,oo.managedProjectId,eo.projectId,eo.managedProjectId,rp.projectId,rp.managedProjectId),expires:toTs(oo.expires||eo.expires),clientId:pickS(gc.clientId,ec.clientId),clientSecret:pickS(gc.clientSecret,ec.clientSecret)};}"
        "function rAnti(){for(const f of AG){const data=rJson(f);const root=asO(data);const accs=Array.isArray(root&&root.accounts)?root.accounts:[];if(!accs.length)continue;const idx=typeof(root&&root.activeIndex)==='number'?root.activeIndex:0;const acc=asO(accs[idx])||asO(accs[0]);if(!acc)continue;const rp=pgR(acc.refreshToken);const ac=gCred(acc,'ANTIGRAVITY');const rc=gCred(root,'ANTIGRAVITY');if(!rp.refreshToken&&!pickS(acc.accessToken,acc.access))continue;return{sourceId:'antigravity',sourceLabel:'Antigravity',accessToken:pickS(acc.accessToken,acc.access),refreshToken:rp.refreshToken,projectId:pickS(acc.projectId,acc.managedProjectId,rp.projectId,rp.managedProjectId),expires:toTs(acc.expires),clientId:pickS(ac.clientId,rc.clientId),clientSecret:pickS(ac.clientSecret,rc.clientSecret),email:pickS(acc.email)};}}return null;}"
        "async function rGAccess(src){if(src.accessToken&&(!(typeof src.expires==='number')||src.expires>Date.now()))return src.accessToken;if(!src.refreshToken||!src.clientId||!src.clientSecret)return null;try{const res=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({client_id:src.clientId,client_secret:src.clientSecret,refresh_token:src.refreshToken,grant_type:'refresh_token'})});if(!res.ok)return null;const d=await res.json();return asS(d&&d.access_token);}catch{return null;}}"
        "async function fGQB(t,pid){const body=pid?{project:pid}:{};try{const ac=typeof AbortController!=='undefined'?new AbortController():null;const tm=ac?setTimeout(()=>ac.abort(),15000):null;const res=await fetch('https://cloudcode-pa.googleapis.com/v1internal:retrieveUserQuota',{method:'POST',headers:{Authorization:'Bearer '+t,'Content-Type':'application/json'},body:JSON.stringify(body),signal:ac&&ac.signal});if(tm)clearTimeout(tm);if(!res.ok)return null;return await res.json();}catch{return null;}}"
        "async function fGM(t,pid){const body=pid?{project:pid}:{};for(const ep of GEP){try{const ac=typeof AbortController!=='undefined'?new AbortController():null;const tm=ac?setTimeout(()=>ac.abort(),15000):null;const res=await fetch(ep+'/v1internal:fetchAvailableModels',{method:'POST',headers:Object.assign({Authorization:'Bearer '+t,'Content-Type':'application/json'},GHD),body:JSON.stringify(body),signal:ac&&ac.signal});if(tm)clearTimeout(tm);if(res.ok)return await res.json();}catch{}}return null;}"
        "async function fG(a){const srcs=[];const gm=rGem(a);if(gm)srcs.push(gm);const ag=rAnti();if(ag)srcs.push(ag);if(!srcs.length)return null;const models={};const errs=[];for(const src of srcs){const tok=await rGAccess(src);if(!tok){errs.push(src.sourceLabel+': Missing usable host OAuth data');continue;}let merged=false;if(src.sourceId==='gemini'){const qp=await fGQB(tok,src.projectId);const buckets=Array.isArray(qp&&qp.buckets)?qp.buckets:[];for(const b of buckets){const modelId=pickS(b&&b.modelId);if(!modelId)continue;const remF=toN(b&&b.remainingFraction);const remP=remF!==null?Math.round(remF*100):null;const used=remP!==null?Math.max(0,100-remP):null;const rA=toTs(b&&b.resetTime);const w=gWin(src.sourceId,rA);const scoped=modelId.startsWith(src.sourceId+'/')?modelId:(src.sourceId+'/'+modelId);models[scoped]={windows:{}};models[scoped].windows[w.label]=tUW({uP:used,wS:w.seconds,rA:rA});merged=true;}}const mp=await fGM(tok,src.projectId);const raw=mp&&typeof mp==='object'&&mp.models&&typeof mp.models==='object'?mp.models:{};for(const key of Object.keys(raw)){const md=raw[key]||{};const qi=md&&md.quotaInfo&&typeof md.quotaInfo==='object'?md.quotaInfo:{};const remF=toN(qi.remainingFraction);const remP=remF!==null?Math.round(remF*100):null;const used=remP!==null?Math.max(0,100-remP):null;const rA=toTs(qi.resetTime);const w=gWin(src.sourceId,rA);const scoped=key.startsWith(src.sourceId+'/')?key:(src.sourceId+'/'+key);models[scoped]={windows:{}};models[scoped].windows[w.label]=tUW({uP:used,wS:w.seconds,rA:rA});merged=true;}if(!merged)errs.push(src.sourceLabel+': No quota models visible');}if(!Object.keys(models).length)return bR({pId:'google',pName:'Google',ok:false,err:errs[0]||'Failed to fetch Google quota'});return bR({pId:'google',pName:'Google',ok:true,use:{windows:{},models:models}});}"
        ...
        '(async()=>{const a=rAuth();const authKeys=Object.keys(a);const R=[];const c=await fC(a);if(c)R.push(c);const o=await fO(a);if(o)R.push(o);const x=await fX(a);if(x)R.push(x);const g=await fG(a);if(g)R.push(g);const gh=await fGH(a);if(gh)R.push(gh);'
        "const unsupported=authKeys.filter(k=>!['anthropic','claude','openrouter','openai','codex','chatgpt','google','google.oauth','github-copilot','copilot'].includes(k));"
