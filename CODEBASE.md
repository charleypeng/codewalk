...
lib/domain/entities/experience_settings.dart                    # Persisted terminal visibility, height, maximize state, read-aloud settings (readAloudEnabled, readAloudRate, readAloudPitch, readAloudVoice), debug logging toggles (`loggingEnabled`, `performanceLoggingEnabled`, both default false), and **visual style** (`VisualStyle visualStyle`, default `VisualStyle.refined` for new installs, parsed via `visualStyleKey`/`visualStyleFromKey`) inside shared experience settings; `fromJson` migration upgrades older payloads — when `loggingEnabled` is missing but a legacy `performanceLoggingEnabled` is true, logging is implicitly enabled so the older diagnostic preference keeps working, and persisted JSON missing the `visualStyle` key falls back to `VisualStyle.classic` for backward compatibility
