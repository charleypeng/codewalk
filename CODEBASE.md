...
lib/core/auth/oauth_service_io.dart                # OAuthService IO implementation (desktop + Android): Cloudflare Access Managed OAuth with PKCE (S256); desktop (system browser) and Android (in-app WebView) both use an in-app local HttpServer loopback callback (Cloudflare DCR only allows loopback redirect URIs); credential caching/refresh, OAuth metadata discovery, trusted endpoint validation, callback URL validation
