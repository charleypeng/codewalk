...
lib/core/auth/oauth_service_io.dart                # OAuthService IO implementation (desktop + Android): Cloudflare Access Managed OAuth with PKCE (S256); desktop and Android both use an in-app local HttpServer loopback callback (Cloudflare DCR only allows loopback redirect URIs); credential caching/refresh, OAuth metadata discovery, trusted endpoint validation, callback URL validation
...
- Cloudflare Access OAuth supported on Android and desktop via the same in-app local HTTP redirect server (loopback callback); Cloudflare Managed OAuth DCR only accepts loopback redirect URIs, so no custom-scheme/AppAuth redirect is used.`
...
- **Tests**: `test/unit/presentation/workspace_file_operations_service_test.dart` covers write-path validation, 48 KiB content chunking, and negotiated GNU/BSD/Python decoder selection. `test/integration/workspace_file_operations_live_test.dart` is an opt-in local OpenCode probe/create/write/read/delete integration test. `test/support/fakes.dart` adds `writeFileResult`, `writeFileCallCount`, and `onWriteFile` hook to `FakeWorkspaceFileOperationsService`. `test/widget/chat_page_test.dart` covers file-editor save, CRLF round-trip, current-draft Add-to-chat, dirty-state failure/close/rename guards, and editable empty text files.
