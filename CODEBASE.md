...
- Visual style layer (issue #86): `VisualStyle` (`classic` / `refined`) persisted in `ExperienceSettings`, exposed via `SettingsProvider`, and propagated through `AppVisualStyleTokens` `ThemeExtension` so chat surfaces can consume shape/surface tokens while `OpenCodeThemeTokens` continue to drive markdown/syntax palettes.
...
```text
lib/core/di/injection_container.dart              # Registers datasources, repositories, usecases, providers, TailscaleService, and WorkspaceFileOperationsService (issue #89); wires tailscaleService into AppProvider factory; _loadLocalConfig applies tailscaleEnabled on active profile
...
lib/presentation/providers/settings_provider.dart # Experience settings, theme mode, dynamic color, AMOLED dark toggle, brand seed, contrast, **Visual style** (`visualStyle` getter + `setVisualStyle()` persisting `VisualStyle.classic`/`refined`, default `classic`), composer tips visibility, sounds, update checks, complete OpenCode shared settings coverage (default model, default agent, small model, autoupdate, share, username, snapshot), and debug logging toggles (`loggingEnabled` default off, `performanceLoggingEnabled` default off); exposes `dynamicColorAvailable` (bool) and `updateDynamicColorAvailability()` for runtime platform signal; `setCheckUpdatesOnOpen()` now controls startup + hourly automatic checks via `_configureAutomaticUpdateChecks()` and `_performStartupUpdateCheck()`; `UpdateInstallState` enum (idle/downloading/installing/done/failed), `startInstall()`, and `restartDesktopApp()` manage APK/desktop install lifecycle; `initialize()` syncs persisted logging preferences into AppLogger via `setLoggingEnabled`/`setPerformanceLoggingEnabled`; `setLoggingEnabled()` and `setPerformanceLoggingEnabled()` update ExperienceSettings, push to AppLogger, and persist
...
lib/presentation/theme/app_theme.dart                 # Material You theme builder using AppShapes, color scheme, and `AppVisualStyleTokens` (issue #86); `lightFrom`/`darkFrom` accept an optional `visualStyle` (default `VisualStyle.classic`) and forward it to `_buildTheme` which mounts `AppVisualStyleTokens.classic` or `AppVisualStyleTokens.refined` into the theme extensions alongside caller-supplied `OpenCodeThemeTokens`; `withResponsiveSnackBars` derives the snackbar shape from `theme.visualStyleTokens.controlRadius` when refined (and falls back to `AppShapes.borderLarge` when classic); also houses AppDensitySpacing (density-aware spacing for chrome/composer)
...
lib/presentation/services/workspace_file_operations_service.dart # WorkspaceFileOperationsService (issue #89 Files micro file manager + issue #90 file editor): shell-gated `createFolder`/`createFile`/`rename`/`delete`/`writeFile` with capability probe (`CW_FILE_OP_JSON:` sentinel) and ephemeral `/session` lifecycle; `writeFile` (issue #90) ships UTF-8 text content from the editor as base64 over the shell transport, decodes into a per-call `mktemp -d "$parent/.cw-write.XXXXXX"` temp directory at `$tmpdir/content`, calls `cw_copy_mode` to mirror the existing file's mode onto the staged file, then `mv -- "$tmp" "$target"` to swap into place (with a `cw_fail failed` guard if `mktemp`/decode fails); `WorkspaceFileOperationCode` enum (`ok`/`unavailable`/`invalidName`/`outsideRoot`/`rootDeleteBlocked`/`missing`/`alreadyExists`/`permissionDenied`/`notDirectory`/`failed`/`malformedResponse`); `WorkspaceFileOperationsCapabilities(shellFileOpsSupported, message)` cache-scoped per `serverScopeKey::directory`; scripts use `set -u` + `cw_validate_name`/`cw_prepare_parent`/`cw_decode_content`/`cw_copy_mode` helpers and the sentinel line is extracted from the last matching string anywhere in the nested shell response
...
chat_page_file_viewer.dart                       # File editor surface (issue #90): renders an `re_editor` `CodeEditor` with `re_highlight` language modes and per-file `CodeHighlightTheme`; tracks draft via `_editorDraftForContent`, exposes dirty marker `*` on tab header (value key `file_viewer_tab_dirty_{normalizedPath}`), Save button + Ctrl/Cmd+S shortcut, read-only gating (`_editorReadOnlyReason`) for files exceeding `_maxEditableFileLength` (1 MiB) or when shell file ops are unavailable/loading, inline save error banner, open-file scroll-to-line handling via `pendingScrollToLine`, and a clickable gutter (`file_editor_gutter_{path}`) with `_EditorLineSelectionPainter` that restores the persisted `selectedLinesByPath`/`lastSelectedLineByPath` selection across rebuilds via shift-click range toggle (`_handleGutterLineTap`)
...
lib/presentation/widgets/chat_message/chat_message_content.dart     # Message bubble layout, copy/hold layers, latest inline undo, historical inline rewind button, and read-aloud button in assistant message header (volume_up/stop icon, ListenableBuilder, markdown stripping); consumes `theme.visualStyleTokens` (issue #86)
...
lib/domain/entities/experience_settings.dart                    # Persisted terminal visibility, height, maximize state, read-aloud settings (readAloudEnabled, readAloudRate, readAloudPitch, readAloudVoice), and debug logging toggles (`loggingEnabled`, `performanceLoggingEnabled`, both default false) inside shared experience settings; `fromJson` migration upgrades older payloads — when `loggingEnabled` is missing but a legacy `performanceLoggingEnabled` is true, logging is implicitly enabled so the older diagnostic preference keeps working
...
test/unit/domain/experience_settings_test.dart # `ExperienceSettings` JSON round-trip covers `visualStyle` (`VisualStyle.classic`/`refined`) including unknown-key fallback (issue #86)
...
test/unit/providers/                   # ChatProvider split tests (8 files, parallelized with -j 12); `settings_provider_test.dart` covers `setVisualStyle()` persistence + reload (issue #86)
...
test/unit/presentation/                 # Presentation-level service tests; includes `workspace_file_operations_service_test.dart` (issues #89 and #90) covering sentinel payload extraction (`extractSentinelPayload` traverses nested shell parts), malformed-response parsing, shell-quoting (`shellQuoteForTest`), probe command generation, create/rename/delete end-to-end behavior with ephemeral session teardown, and `writeFile` base64 transport + outside-root/root-blocked guards; the `write file uses base64 content and returns target path` test asserts the staged-write shell command contains `mktemp -d`, the `.cw-write.XXXXXX` pattern (not the legacy `.cw-write-$$.tmp`), `cw_decode_content "$tmp"`, `cw_copy_mode "$target" "$tmp"`, and the final `mv -- "$tmp" "$target"` swap; `app_theme_test.dart` (issue #86) covers `AppVisualStyleTokens.classic`/`refined` factories, theme-extension wiring through `AppTheme.lightFrom`/`darkFrom`, `withResponsiveSnackBars` shape switching, and the `ThemeData.visualStyleTokens` fallback getter
...
- **Visual style layer (issue #86)**: `VisualStyle` (`classic` / `refined`, default `classic`) is
  persisted via `ExperienceSettings` (`visualStyleKey`/`visualStyleFromKey` for JSON) and exposed
  by `SettingsProvider.visualStyle` + `setVisualStyle()`. `main.dart` reads `settingsProvider.visualStyle`
  and forwards it to `AppTheme.lightFrom`/`darkFrom` together with the resolved `OpenCodeThemeTokens`,
...
- **Service** (`lib/presentation/services/workspace_file_operations_service.dart`): `WorkspaceFileOperationsService` exposes `getCapabilities` (per `serverScopeKey::directory` cache), `invalidateCapabilities`, and the four mutations `createFile`/`createFolder`/`rename`/`delete`. Each shell probe or mutation uses an ephemeral session (`/session`, then `DELETE /session/:id` plus a 5s delayed removal from `ChatTitleGenerator.ephemeralSessionIds`) and POSTs either the probe command or a `set -u` shell script to `/session/:id/shell` (`agent: build`) with the active project `directory` query. Scripts use shared `cw_validate_name`/`cw_prepare_parent` helpers and the last `CW_FILE_OP_JSON:` line emits the operation result as a JSON envelope with `ok`/`code`/`message`/`path`/`newPath`; the sentinel is extracted from anywhere in the nested shell response. The capability probe resolves the server-side working directory with `pwd -P` and records `shellFileOpsSupported`; empty/literal `/` roots are rejected before probing, canonical `pwd -P` `/` returns `outsideRoot`, and mutation results with `unavailable`/`malformedResponse` invalidate the cache.
...
- **Service write path** (`workspace_file_operations_service.dart`): `WorkspaceFileOperationsService.writeFile({serverScopeKey, rootDirectory, path, content})` reuses `_preparePathOperation` validation (outside-root + root-blocked), then runs `_buildWriteFileCommand` which carries `content` as `base64Encode(utf8.encode(content))` to the same ephemeral `/session/:id/shell` transport used by the mutations; the staged-write script creates a per-call `mktemp -d "$parent/.cw-write.XXXXXX"` temp directory, decodes the payload into `$tmpdir/content` via `cw_decode_content`, mirrors the existing file's mode with `cw_copy_mode "$target" "$tmp"`, then `mv -- "$tmp" "$target"` to swap into place (with `cw_fail failed` if `mktemp` or decode fails). Existing capability cache, mutation scaffolding, and result-code mapping are shared.
- **Draft state** (`chat_page_local_models_part.dart`): `_FileEditorDraftState` owns the `CodeLineEditingController.fromText` + `CodeScrollController` for a single path, tracks `savedContent`, `isSaving`, `saveErrorMessage`, exposes `isDirty` via `controller.text != savedContent`, and provides `markSavedContent` / `replaceSavedContent` / `dispose`. `_FileExplorerContextState.editorDraftsByPath` is the per-context map; `resetForRoot` and `dispose` clear/dispose drafts, `removeEditorDraft` disposes a single entry.
- **Editor UI** (`chat_page_file_viewer.dart`): Tab header gets a dirty marker `*` when `_fileDraftIsDirty`; the focused viewer wraps a `CodeEditor` styled with `CodeEditorStyle` (monospace font, theme colors, line number gutter) and a `CodeHighlightTheme` keyed on the resolved language with `_maxHighlightedFileLength` ceiling. Save button (`file_viewer_save_button`) calls `_saveFileEditorDraft`; `Ctrl/Cmd+S` shortcuts route to the same handler when the viewer has focus. `_editorReadOnlyReason` returns a string when (a) `utf8.encode(content).length > _maxEditableFileLength` (1 MiB) — files open read-only for responsiveness, (b) capabilities are still loading, or (c) `shellFileOpsSupported != true` — the resolved message (or `filesOperationUnavailable`) is shown in an inline read-only banner. Empty non-binary files (`content.isEmpty`) are **not** gated out by the size check, so they open directly as editable drafts. `_canSaveFileDraft` requires dirty + not saving + UTF-8 byte length within `_maxEditableFileLength` + supported shell ops. Save gating (`_canSaveFileDraft`) and the save pipeline both gate on the current draft's UTF-8 byte length via `_isEditorContentTooLarge(draft.controller.text)`; when the gate trips, the static `_draftTooLargeSaveMessage` (`Draft is too large to save from the editor.`) is shown inline and surfaces as a snackbar. `pendingScrollToLine` is a general open-file scroll-to-line target on `_FileExplorerContextState` — when set (e.g. by open-with-line), the viewer clears it and schedules a `makeCenterIfInvisible` on the draft's scroll controller after the first frame.
...
- **Save pipeline** (`chat_page_file_runtime.dart`): `_saveFileEditorDraft` short-circuits when not dirty or already saving; checks DI registration and `_fileMutationsSupported` (snackbars `filesOperationUnavailable` if unsupported); gates on `utf8.encode(contentToSave).length > _maxEditableFileLength` and surfaces `_ChatPageFileViewer._draftTooLargeSaveMessage` (`Draft is too large to save from the editor.`) inline and via snackbar; sets `isSaving=true` + clears error; awaits `WorkspaceFileOperationsService.writeFile(serverScopeKey, rootDirectory, path, content)`; on failure sets `saveErrorMessage` via `_fileOperationErrorLabel(result.code)` and surfaces a snackbar; on success calls `markSavedContent`, updates the cached `_FileTabViewState` (preserving mimeType), and snackbars success. The handler aborts silently if the draft was swapped or the page was unmounted mid-flight.
- **Dirty-aware reconciliation**: `_reloadFileTab` blocks dirty editor drafts before a reload can swap content — paths with dirty drafts skip the swap and set `saveErrorMessage` to the static string `Unsaved changes; reload skipped.`; diff-aware silent reloads reach that same guard via `_reconcileFileContextWithSessionDiff`, which invokes `_reloadFileTab(silent: true)` per matched tab path. `_reconcileRenamedFileTreePath` remaps `editorDraftsByPath` keys via `_replacePathPrefix`; `_reconcileDeletedFileTreePath` removes and disposes drafts for the deleted path or subtree, and clears `tabSelection.activePath` if the active tab was removed.
- **Tests**: `test/unit/presentation/workspace_file_operations_service_test.dart` adds `write file uses base64 content and returns target path` (asserts the command contains `mktemp -d`, the `.cw-write.XXXXXX` pattern, `cw_decode_content "$tmp"`, `cw_copy_mode "$target" "$tmp"`, and `mv -- "$tmp" "$target"`, and explicitly that the legacy `.cw-write-$$.tmp` marker is absent), `write file rejects paths outside root before shell calls` (relative + absolute), `write file blocks project root before shell calls`, and `write file malformed operation invalidates capability cache`. `test/support/fakes.dart` adds `writeFileResult`, `writeFileCallCount`, and `onWriteFile` hook to `FakeWorkspaceFileOperationsService`. `test/widget/chat_page_test.dart` adds `file editor saves dirty content from open files dialog` (controller edit + Ctrl/Cmd+S), `file editor preserves CRLF line endings when saving` (CRLF input round-trips through save), `file editor keeps dirty state when save fails` (button-save failure surfaces inline error), `file editor opens empty text files as editable drafts` (tapping a known empty non-binary file confirms the editor renders with the editable `CodeEditor`), `file editor gutter selection adds current draft to chat context` (gutter line selection reads the live draft text and emits `FileInputPart` entries with `?start=&end=` URLs), `file editor blocks closing dirty tabs` (close on a dirty draft is refused with `Save changes before closing this file.`), and `file tree rename blocks dirty relative editor drafts` (rename attempt on a relative-path dirty draft aborts via `_blockPathMutationForActiveEditorDrafts`, surfacing `Save changes before changing this path.` and keeping the draft + tab intact).
