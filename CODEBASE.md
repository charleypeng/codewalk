...
lib/presentation/providers/project_icon_provider.dart # Client-owned project icon orchestration (ADR-040, issue #73): loads cached icons from `ProjectIconStore`, runs user-initiated discovery via `ProjectIconDiscoveryService`, and exposes per-project `iconFor`/`isLoading`/`isDiscovering` state; OpenCode project payloads remain authoritative/unchanged
...
lib/presentation/widgets/project_icon.dart # `ProjectIcon` widget renders cached `ProjectIconData` (PNG/JPEG/WebP via `Image.memory`, SVG via `flutter_svg`, ICO normalized to PNG) without recoloring stored artwork; falls back to tinted `Symbols.folder_open` when no icon is cached or rendering fails. `ProjectIconDiscoveryButton` is the user-initiated discovery trigger that calls `ProjectIconProvider.discoverIcon` and surfaces `ProjectIconDiscoveryResult` via `onResult`
...
lib/presentation/pages/chat_page_local_models_part.dart # Local UI state classes (part of chat_page.dart; see commit 8759defc); `_FileExplorerContextState` carries `WorkspaceFileOperationsCapabilities?` plus `fileOperationCapabilitiesLoading` flag, `rootDirectory`, `directoryChildren`, `expandedDirectories`, `loadingDirectories`, `directoryErrors`, `tabsByPath`, `FileTabSelectionState`, line-selection maps (`selectedLinesByPath`, `lastSelectedLineByPath`), `pendingScrollToLine`, `rootLoadScheduled`, and `treeError`; `resetForRoot(nextRootDirectory)` clears capabilities + tab selection on context switch (issue #89)
