...
lib/presentation/services/read_aloud_service.dart                # ReadAloudService: provider-routed read-aloud facade; selects native/generated-audio TTS backends, loads the secure OpenAI-compatible API key when needed, tracks idle/playing/paused state and per-message playback
...
lib/presentation/services/tts/edge_experimental_tts_backend.dart # Experimental Microsoft Edge voice-list adapter; direct synthesis remains blocked in this build
...
lib/presentation/widgets/chat_message_widget.dart # Message bubble with build-skip cache, cached MarkdownStyleSheet, provider-aware sanitized read-aloud controls, compact collapsed-copy variants, task navigation callbacks, inline undo/revert, clickable file paths, and Mermaid routing; consumes `theme.visualStyleTokens` (issue #86)
...
lib/presentation/widgets/chat_message/chat_message_content.dart     # Message bubble layout, copy/hold layers, inline undo/rewind, and provider-aware read-aloud button that sanitizes assistant text before routing to ReadAloudService; consumes `theme.visualStyleTokens` (issue #86)
...
- **Backends**: `ReadAloudService` routes `ReadAloudProvider.native` to `NativeTtsBackend` and generated-audio providers to `TtsBackend` adapters plus `TtsAudioPlayer` byte playback.
