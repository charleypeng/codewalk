...
- [x] Investigate and fix conversation continuity when switching sessions/projects... - Related commits: acce617 9dcd773 1581c65 cdee253 1fcf33e 68baebe 61934e9 0ee474c df9ec9e 931d9a8
...
- [x] Corrigir reconciliação de IDs que duplica mensagem do usuário: em alguns envios, a mensagem do usuário é duplicada e, após a resposta final do assistente, uma cópia reaparece fora de ordem. Suspeita de falha na reconciliação/chave de ID no estado da conversa. DoD: IDs únicos e estáveis por mensagem sem duplicação após resposta. - Commit hash: 672ee9d
- [x] Revamp do question tool com fluxo em steps e layout mobile-first. Hoje cada opção tem descrição no payload, mas o app não exibe esse detalhe; em múltiplas perguntas o conteúdo pode estourar no celular e ocultar o botão de resposta. Implementar wizard em etapas (uma pergunta por vez), seleção/avanço por step e envio consolidado no passo final, além de revisão visual para organização e atratividade. DoD: 1) descrições das opções visíveis quando fornecidas; 2) fluxo multi-pergunta em steps com botão final de envio; 3) layout responsivo sem perda de CTA em telas pequenas; 4) visual atualizado e consistente com UX do app.
...
- [ ] Pesquisar em detalhe como OpenCode CLI e OpenCode Web tratam envio de mensagem com processo em andamento (fila): mapear gatilhos de enfileiramento, política de ordem/prioridade, feedback de UX no composer/chat, comportamento de cancelar/editar/reenviar, persistência da fila em reconexão/restart e casos de borda (falha de rede, troca de sessão/projeto, múltiplas mensagens).
- [ ] Canned answers manager for fast reply
- [x] Quando o app mostra o pedido de permissão de acesso a pastas, o balão aparece duas vezes, um em cima e uma mais embaixo, basta responder um que ambos somem. O bug é que deveria aparecer só 1. Uma forma fácil de aparecer é tentar acessar /tmp/ - Commit hash: 5733b01

- [x] Auto-colapsar painel de tasks com teclado no mobile preservando estado anterior: ao abrir teclado no celular, colapsar temporariamente o widget de tasks (todowrite) para liberar área útil. Ao fechar teclado, restaurar exatamente o estado anterior ao foco de input (expanded/collapsed), sem mudar preferência do usuário. DoD: 1) teclado aberto no mobile => painel colapsa automaticamente; 2) teclado fechado => painel retorna ao estado anterior; 3) se painel já estava colapsado, permanece colapsado; 4) se estava expandido, reexpande; 5) consistente em Android e iOS.
- [ ] Suavizar efeito de varredura do status no composer: ajustar a animação para que a varredura ultrapasse levemente o início e o fim do texto (overscan), evitando corte abrupto na última letra e reinício visível no começo. DoD: 1) sweep continua além da última letra antes de desaparecer; 2) início também recebe overscan para entrada suave; 3) transição de loop sem salto perceptível; 4) comportamento consistente em mobile e desktop.
...
**Post-conclusion**: Performance overhaul eliminating freeze/lag in streaming and large conversations (microtask coalescing, event dedup buffer, timeline/highlight/style caches, build-skip widget conversion, lazy copy text) in `00b326f`. Upgraded flutter_secure_storage 9.2.4 → 10.0.0 (Java 17, linux 3.0.0) in `547da29`. Memoized `currentThreadPermissionRequests` to avoid redundant recomputation during streaming in `ed6d4cc`.
...
- [x] Eliminate dead code and analyzer warnings — Commit: 51757e8. Completed. Removed ~210 lines of dead code: 4 superseded sync methods, 1 unused method, 1 unused constructor+enum+switch arm, 1 unused import, 1 write-only field. Analyzer: 0 warnings (was 8), issues 182 → 174.
