...
- [x] Corrigir o erro ao tentar enviar mensagem após resposta do assistente (hidden response until session switch/retry): implemented async send stale-ID protection and idle final-reconcile bypass during abort suppression. - Related commits: 745c0a8 f1faf4a 4074734 5156e1f f0fda55 92656f3 6443d2e f6376f6
...
- [x] Align chat optimistic user-message identity with server `messageID` contract: implemented `msg_*` ID generation and forwarding to prevent recurring duplicate user bubble regressions under refresh/realtime merges. - Related commits: b0660a2 e92c3f8
- [ ] Analisar como reduzir o consumo de dados do App em conexões limitadas como 5G, em poucos minutos consome GB de dados, e não está claro o motivo, além de gastar muitos dados do usuário, pode drenar a bateria tornando o app em si um problema de uso móvel
- [ ] Reduzir da lista de Conversations também no desktop
